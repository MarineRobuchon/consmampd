...
##  revised March 2015                          ##
##  re-revised by Marine Robuchon, July 2019    ##
##  only line 37 was modified                   ##
##################################################
...
calc_PE <- function(tree, sites_x_tips,presence=c("presence","abundance","probability")) {
  ...
  for (i in 1:nTips(tree)) {
    sites_x_branches[,i] <- sites_x_tips[,which(labels(tree)[i]==labels(sites_x_tips)[[2]])] # this is what I changed compared to the original function by DR, because names(sites_x_tips) was null
